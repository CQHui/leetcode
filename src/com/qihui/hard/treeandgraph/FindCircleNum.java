...
public class FindCircleNum {
    ...
    public int findCircleNum(int[][] isConnected) {
        ...
        for (int i = 0; i < isConnected.length; i++) {
            for (int j = i; j < isConnected.length; j++) {
                if (isConnected[i][j] == -1) break;
                if (j > 0 && isConnected[i][j - 1] == -1) {
    ...
    public boolean dfs(int i, int j) {
        ...
        int index = j;
        while (index > 0) {
            if (isConnected[i][index - 1] == 1) {
                dfs(i, index - 1);
            }
            isConnected[i][index - 1] = -1;
            index -= 1;
        }
        index = j;
        while (index < isConnected.length - 1) {
            if (isConnected[i][index + 1] == 1) {
                dfs(i, index + 1);
            }
            isConnected[i][index + 1] = -1;
